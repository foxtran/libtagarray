...
Record::Record(const uint32_t type_id,
  ...
  std::copy(dimensions_ptr, dimensions_ptr + this->_dimensions.size(), this->_dimensions.begin());
  std::copy(options_ptr, options_ptr + this->_options.size(), this->_options.begin());
...
Record::~Record() noexcept { if(this->_data != nullptr) delete[] this->_data; }
...
void Record::dump(const int32_t level) const noexcept {
  ...
  if (level > 0) {
    ...
    switch(this->_type_id) {
      case TA_TYPE_CHAR8:
        for (size_t i = 0; i < (this->_data_length / sizeof(char)) && i < (size_t)level; i++) {
          std::cout << (char)(reinterpret_cast<char*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_INT8:
        for (size_t i = 0; i < (this->_data_length / sizeof(int8_t)) && i < (size_t)level; i++) {
          std::cout << (int8_t)(reinterpret_cast<int8_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_INT16:
        for (size_t i = 0; i < (this->_data_length / sizeof(int16_t)) && i < (size_t)level; i++) {
          std::cout << (int16_t)(reinterpret_cast<int16_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_INT32:
        for (size_t i = 0; i < (this->_data_length / sizeof(int32_t)) && i < (size_t)level; i++) {
          std::cout << (int32_t)(reinterpret_cast<int32_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_INT64:
        for (size_t i = 0; i < (this->_data_length / sizeof(int64_t)) && i < (size_t)level; i++) {
          std::cout << (int64_t)(reinterpret_cast<int64_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_UINT8:
        for (size_t i = 0; i < (this->_data_length / sizeof(uint8_t)) && i < (size_t)level; i++) {
          std::cout << (uint8_t)(reinterpret_cast<uint8_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_UINT16:
        for (size_t i = 0; i < (this->_data_length / sizeof(uint16_t)) && i < (size_t)level; i++) {
          std::cout << (uint16_t)(reinterpret_cast<uint16_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_UINT32:
        for (size_t i = 0; i < (this->_data_length / sizeof(uint32_t)) && i < (size_t)level; i++) {
          std::cout << (uint32_t)(reinterpret_cast<uint32_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_UINT64:
        for (size_t i = 0; i < (this->_data_length / sizeof(uint64_t)) && i < (size_t)level; i++) {
          std::cout << (uint64_t)(reinterpret_cast<uint64_t*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_REAL32:
        for (size_t i = 0; i < (this->_data_length / sizeof(float)) && i < (size_t)level; i++) {
          std::cout << (float)(reinterpret_cast<float*>(this->_data))[i] << " ";
        }
        break;
      case TA_TYPE_REAL64:
        for (size_t i = 0; i < (this->_data_length / sizeof(double)) && i < (size_t)level; i++) {
      ...
      default:
        for (size_t i = 0; i < (this->_data_length / sizeof(int8_t)) && i < (size_t)level; i++) {
          std::cout << std::hex << (int)reinterpret_cast<uint8_t*>(this->_data)[i] << " ";
        }
        std::cout << std::dec;
        break;
    }
