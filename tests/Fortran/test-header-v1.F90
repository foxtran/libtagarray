...
contains
  integer function check_container() result(status)
    ...
    call container%new()
    TA_CONTAINER_CREATE(stat, container, "INT32-value", TA_TYPE_INT32, value)
    if (stat /= TA_OK) then
      status = 2
      return
    end if
    TA_CONTAINER_CREATE(stat, container, "INT32-array", TA_TYPE_INT32, array)
    if (stat /= TA_OK) then
      status = 3
      return
    end if
    TA_CONTAINER_CREATE_OPTIONAL(stat, container, "INT32-value-opt", TA_TYPE_INT32, value, description = "value")
    if (stat /= TA_OK) then
      status = 4
      return
    end if
    TA_CONTAINER_CREATE_OPTIONAL(stat, container, "INT32-array-opt", TA_TYPE_INT32, array, description = "array")
    if (stat /= TA_OK) then
      status = 5
      return
    end if
    ...
    TA_CONTAINER_GET_ARRAY(container, "INT32-array", TA_TYPE_INT32, p_array, stat)
    if (stat /= TA_OK) then
      status = 9
      return
    end if
    ...
    TA_CONTAINER_GET_ARRAY(container, "INT32-array-opt", TA_TYPE_INT32, p_array, stat)
    if (stat /= TA_OK) then
      status = 14
      return
    end if
