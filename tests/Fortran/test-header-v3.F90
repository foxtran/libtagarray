#define TA_FORTRAN_API_VERSION 3
#include "tagarray.fh"
...
contains
  integer function check_container() result(status)
    ...
    TA_CONTAINER_ADD_VALUE(stat, container, "INT32-value-opt", value, comment = "value")
    if (stat /= TA_OK) then
      status = 4
      return
    end if
    TA_CONTAINER_ADD_DATA(stat, container, "INT32-array-opt", array, comment = "array")
    if (stat /= TA_OK) then
      status = 5
      return
    end if
    lstat = container%has_records((/ character(len=20) :: "INT32-value", "INT32-array", "INT32-value-opt", "INT32-array-opt" /))
    if (.not.lstat) then
      status = 6
      return
    end if
